...
use feos_core::{parameter::Parameter, EosError, EosResult, HelmholtzEnergy, Residual};
use ndarray::Array1;
...
mod test {
    ...
    use feos_core::{Contributions, State, EquationOfState};
    use ndarray::arr1;
    ...
    fn helmholtz_energy_pure_wca() -> EosResult<()> {
        ...
        let uvtheory = Arc::new(UVTheory::new(Arc::new(parameters))?);
        let eos = Arc::new(EquationOfState::new_default_ideal_gas(uvtheory));
    ...
    fn helmholtz_energy_pure_bh() -> EosResult<()> {
        ...
        let uvtheory = Arc::new(UVTheory::with_options(Arc::new(parameters), options)?);
        let eos = Arc::new(EquationOfState::new_default_ideal_gas(uvtheory));
    ...
    fn helmholtz_energy_mixtures_bh() -> EosResult<()> {
        ...
        let parameters = UVParameters::new_binary(pure_records, None);
        // state
        let reduced_temperature = 4.0;
        ...
        let state_bh = State::new_nvt(&eos, t_x, volume, &moles).unwrap();
        let a_bh = state_bh
            .molar_helmholtz_energy(Contributions::ResidualNvt)
            .to_reduced(RGAS * t_x)
            .unwrap();
    ...
    fn helmholtz_energy_wca_mixture() -> EosResult<()> {
        ...
        let t_x = reduced_temperature * parameters.epsilon_k[0] * KELVIN;
        let reduced_density = 0.9;
        let moles = arr1(&[0.4, 0.6]) * MOL;
        let total_moles = moles.sum();
        let volume = (parameters.sigma[0] * ANGSTROM).powi(3) / reduced_density * NAV * total_moles;

        // EoS
        let uvtheory = Arc::new(UVTheory::new(Arc::new(parameters))?);
        let eos = Arc::new(EquationOfState::new_default_ideal_gas(uvtheory));
        let state_wca = State::new_nvt(&eos, t_x, volume, &moles).unwrap();
        let a_wca = state_wca
            .molar_helmholtz_energy(Contributions::ResidualNvt)
            .to_reduced(RGAS * t_x)
            .unwrap();
